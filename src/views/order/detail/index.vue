  ...
  <div class="detail-container">
    <div>
      <el-steps :active="formatStepStatus(order.status)" finish-status="success" align-center>
        <el-step :description="formatTime(order.createTime)" title="提交订单"/>
        <el-step :description="formatTime(order.paymentTime)" title="支付订单"/>
        <el-step :description="formatTime(order.deliveryTime)" title="平台发货"/>
        <el-step :description="formatTime(order.receiveTime)" title="确认收货"/>
        <el-step :description="formatTime(order.commentTime)" title="完成评价"/>
      </el-steps>
    </div>
